            ...
            <div class="col-xl-12 col-lg-12">
                <div class="card user-card-full">
                    <div class="row m-l-0 m-r-0">
                        <div class="col-sm-4 bg-gradient-info user-profile">
                            <div class="card-block text-center text-white">
                                <div class="m-b-25"> <img src="<?= base_url("assets/img/profile/default.jpg?v2") ?>" class="img-profile rounded-circle" width="100" alt="User-Profile-Image"> </div>
					...
					<form id="UpdateAccount" action="<?= base_url("editing_account") ?>" method="post">
						<div class="card-body">
							...
							if($this->role_id == "1"){
								...
								<div class="form-group row">
									<div class="col-lg-6">
										<label for="id_telegram"><i class="fab fa-telegram fa-fw" style="font-size:13px;" aria-hidden="true"></i> ID Telegram :</label>
										<input type="number" id="id_telegram" placeholder="ID Telegram, Contoh : 123456789" class="form-control" name="id_telegram" value="<?= $data->id_telegram; ?>" required="">
									</div>
									<div class="col-lg-6">
										<label for="id_telegram"><i class="fab fa-telegram fa-fw" style="font-size:13px;" aria-hidden="true"></i> Tipe Pembayaran :</label>
										<select name="type_pembayara" id="type_pembayara" class="form-control">
											<option value="" disabled>- Pilih Tipe -</option>
											<option value="PRABAYAR" <?php if($data->type_pembayaran == "PRAYABAR"){ echo "selected"; } ?>>PRABAYAR</option>
											<option value="PASCABAYAR" <?php if($data->type_pembayaran == "PASCABAYAR"){ echo "selected"; } ?>>PASCABAYAR</option>
										</select>
									</div>
								</div>
								<label class="text-danger m-0">
									<small>Masa Aktif Akun Sampai <?= date("d-M-Y",strtotime($data->expired_date)); ?><br>
									Mohon untuk bisa lakukan pembayaran sesuai pada waktu yang sudah ditentukan. 
									</small>
								</label>
						...
						<div class="card-footer text-right">
							...
							<button class="btn btn-sm btn-primary" id="action"><i class="fas fa-paper-plane" style="font-size:13px" aria-hidden="true"></i> Save</button>
						</div>
					</form>
                </div>
            </div>
