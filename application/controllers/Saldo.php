    ...
    public function topup_saldo()
    {
        ...
        if($this->form_validation->run() === TRUE){
            ...
            if(!$action){
                ...
                $chat_id = $this->model->gd("user","id,id_telegram","id_telegram IS NOT NULL AND role_id = '1' AND id = '".$this->id_mitra."'","row");
                if(!empty($chat_id->id)){
...
Klik Konfirmasi dibawah ini, jika user benar-benar sudah melakukan pembayaran';
    ...
    public function cancel_topup($p)
    {
        ...
        $action = $this->model->update("top_up","id_mitra = '".$this->id_mitra."' AND id = '$id'",$data);
        if(!$action){
            $data_topup = $this->model->gd("top_up","tanggal,id_user,nominal","id_mitra = '".$this->id_mitra."' AND id = '$id'","row");
    ...
    public function konfirmasi_saldo($id_topup,$id_user,$saldo)
    {
        ...
        if(!empty($saldo_sekarang->id)){
            $get_konfirmasi = $this->model->gd("top_up","id","id_mitra = '".$this->id_mitra."' AND id = '$id_topup' AND status = 'Pending'","row");
            if(!empty($get_konfirmasi->id)){
                $status_topup = [
                    "status" => "Sukses",
                ];
                $action = $this->model->update("top_up","id_mitra = '".$this->id_mitra."' AND id = '$id_topup' AND status = 'Pending'",$status_topup);
                if(!$action){
                    ...
                    //LOG ACTIVIT
                    $tanggal_topup = $this->model->gd("top_up","tanggal","id_mitra = '".$this->id_mitra."' AND id = '$id_topup'","row");
    ...
    public function check_saldo()
    {
        ...
        if(!empty($saldo)){
            $fb = [
                "saldo" => $saldo->saldo,
                "saldo_val" => number_format($saldo->saldo,0,"","."),
            ];
        }else{
            $fb = [
                "saldo" =>  0,
