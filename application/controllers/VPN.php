...
class VPN extends MY_Controller
{
    public function save_vpn_master($p)
    {
        if(!empty($this->idusernzm)){
            ...
            $this->form_validation->set_rules("ip_local_0","IP Local","required|trim|xss_clean|integer");
            $this->form_validation->set_rules("ip_local_1","IP Local","required|trim|xss_clean|integer");
            $this->form_validation->set_rules("ip_local_2","IP Local","required|trim|xss_clean|integer");
            $this->form_validation->set_rules("ip_public_0","IP Public","required|trim|xss_clean|integer");
            $this->form_validation->set_rules("ip_public_1","IP Public","required|trim|xss_clean|integer");
            $this->form_validation->set_rules("ip_public_2","IP Public","required|trim|xss_clean|integer");
            $this->form_validation->set_rules("ip_public_3","IP Public","required|trim|xss_clean|integer");
            ...
            if($this->form_validation->run() === TRUE){
                ...
                $data_input = [
                    ...
                    "ip_local" => $this->input->post("ip_local_0").".".$this->input->post("ip_local_1").".".$this->input->post("ip_local_2"),
                    "ip_public" => $this->input->post("ip_public_0").".".$this->input->post("ip_public_1").".".$this->input->post("ip_public_2").".".$this->input->post("ip_public_3"),
            ...
            if(!empty($id_routeros)){
                ...
                if($koneksi_router == "200"){
                    if(!empty($this->idusernzm)){
                        ...
                        if($this->form_validation->run() === TRUE){
                            $check_user = $this->model->gd("data_order","id","id_mitra = '".$this->id_mitra."' AND username = '".$this->input->post("username")."' AND id_server = '$id_server' AND status = 'Aktif'","row");
                            if(empty($check_user->id)){
                                $nomor_order = $this->model->gd("data_order","COUNT(id) as jumlah_order","id_mitra = '".$this->id_mitra."'","row");
                                $harga_vpn = $this->model->gd("vpn_master","harga,ip_public,ip_local","id_mitra = '".$this->id_mitra."' AND id = '".$this->input->post("id_server")."'","row");
                                $address = addressip($nomor_order->jumlah_order,$harga_vpn->ip_local);
                                $dst_port = ip_number($nomor_order->jumlah_order,substr($harga_vpn->ip_local,0,2));
                                ...
                                if($saldo_akhir < 0){
                                    ...
                                    $firewall_nat = $this->router_process("firewall_nat",$data_input_vpn);
                                    if($firewall_nat == "200"){
                                        $ppp_secret = $this->router_process("ppp_secret",$data_input_vpn);
                                        if($ppp_secret == "200"){
                                            $scheduler = $this->router_process("scheduler",$data_input_vpn);
                                            if($scheduler == "200"){
                                                ...
                                                $this->swal("Gagal","VPN Gagal Order, Karena gagal membuat scheduler anda","error");
                                            }
                                        }else{
                                            $this->swal("Gagal","VPN Gagal Order, Karena gagal membuat PPP Secret anda","error");
                                        }
                                    }else{
                                        $this->swal("Gagal","VPN Gagal Order, Karena gagal membuat Firewall NAT anda","error");
                                    }
                                }
                            }else{
        ...
        if(!empty($this->idusernzm)){
            ...
            if($this->form_validation->run() === TRUE){
                ...
                if(!$action){
                    ...
                    $logs = $this->model->insert("log_activity_user",$data_log);
                    $data_user = $this->model->gd("data_order","username,nomor_order,port,password,id_server","id_mitra = '".$this->id_mitra."' AND id = '$id'","row");
                    if(!empty($data_user)){
    ...
    public function delete_vpn($p)
    {
        if(!empty($this->idusernzm)){
            $id = d_nzm($p);
            $data_vpn = $this->model->gd("data_order","harga_beli,tanggal_order,username","id_mitra = '".$this->id_mitra."' AND id = '$id'","row");
            ...
            if($delete_proses == "OK"){
                ...
                $action_del = $this->model->update("data_order","id_mitra = '".$this->id_mitra."' AND id = '$id'",$data_del);
                if(!$action_del){
                    $data_order = $this->model->gd("data_order","username,id_server","id_mitra = '".$this->id_mitra."' AND id = '$id'","row");
    ...
    {
        $data_server = $this->model->gd("api_routeros","id","id_mitra = '".$this->id_mitra."' AND is_active = '1'","result");
	...
	public function check_username_vpn()
	{
		...
		if($this->form_validation->run() === true){
			$id_server = $this->input->post("id_server");
			$username = $this->input->post("username");
			$check_user = $this->model->gd("data_order","id","id_mitra = '".$this->id_mitra."' AND username = '".$username."' AND id_server = '$id_server'","row");
			if(!empty($check_user)){
				$fb = [
					"status" => "success",
				];
